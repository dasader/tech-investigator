...
**KCI 통합** — KCI Open API(`open.kci.go.kr`)는 XML 응답. `articleSearch.kci` **단일 호출**로 메타데이터·DOI·abstract(한/영)를 모두 받으므로 N+1 detail 호출은 불필요(구현 초기에는 detail 호출까지 설계했으나 실제 API 검증 후 제거). 파라미터는 `keyword=<영문 키워드>`. 응답 element 명: `<articleInfo article-id="...">`(속성), `<title-group><article-title lang="english"|"original">`, `<abstract-group><abstract lang="...">`, `<doi>http://dx.doi.org/...</doi>`(URL prefix 자동 strip), `<journalInfo><journal-name>·<pub-year>` 중첩. `kci_agent.py`가 paper dict로 정규화하고 country를 항상 "South Korea" default + `country_lookup_done=True`로 set해 `extraction_agent`의 OpenAlex 재조회를 차단. `KCI_API_KEY`는 **루트 `.env`**에 설정 (docker-compose의 `env_file: .env`가 루트 기준 — `backend/.env.example`은 빌드 컨텍스트에 우연히 들어있을 뿐 실제로 읽히지 않음).
